import { TopDots } from "@/components/root/top-dots";
import { BottomDots } from "@/components/root/bottom-dots";
import { SectionTitle } from "@/components/root/section-title";

export function Faqs() {
  return (
    <section className="relative z-20 overflow-hidden pb-8 pt-20 lg:pb-[50px] lg:pt-[120px] bg-zinc-400/10" id="faqs">
      <div className="container">
        <SectionTitle
          subtitle="FAQs"
          title="Any Questions? Answered"
          paragraph=""
          width="640px"
          center
        />

        <div className="-mx-4 mt-[60px] flex flex-wrap lg:mt-20">
          <div className="w-full px-4 lg:w-1/2">
            <SingleFaq
              question="How does the fee redistribution work?"
              answer="If an attendee doesn't show up, 80% of their fee is redistributed among the attending participants. The remaining 20% covers our platform costs."
            />
            <SingleFaq
              question="What if I need to cancel my attendance?"
              answer="No worries! You cancel your attendance, but your RSVP fee goes to the event attendees."
            />
            <SingleFaq
              question="Is my money safe?"
              answer="Absolutely. All fees are held securely by the system, acting as an escrow. Payments are only processed after the event concludes."
            />
          </div>

          <div className="w-full px-4 lg:w-1/2">
            <SingleFaq
              question="Can I create any type of Event?"
              answer="Yes, there is no restriction on event types, all events ranging from Corporate events like Conferences, to Social events like Birthdays, as well as Weddings and Educational Seminars"
            />
            <SingleFaq
              question="Can I cancel an event?"
              answer="Yes, you can cancel any event you created, and notifications will be sent to all that RSVPd."
            />
            <SingleFaq
              question="When is the money distributed?"
              answer="All deposits and bonuses are distributed at the night of the event."
            />
          </div>
        </div>
      </div>

      <div>
        <TopDots />
        <BottomDots />
      </div>
    </section>
  );
}

const SingleFaq = (props: { question: string; answer: string }) => {
    const { question, answer } = props;
  
    return (
      <>
        <div className="mb-12 flex lg:mb-[70px]">
          <div className="mr-4 flex h-[50px] w-full max-w-[50px] items-center justify-center rounded-xl bg-primary sm:mr-6 sm:h-[60px] sm:max-w-[60px]">
            <svg
              width="32"
              height="32"
              viewBox="0 0 34 34"
              className="fill-current"
            >
              <path d="M17.0008 0.690674C7.96953 0.690674 0.691406 7.9688 0.691406 17C0.691406 26.0313 7.96953 33.3625 17.0008 33.3625C26.032 33.3625 33.3633 26.0313 33.3633 17C33.3633 7.9688 26.032 0.690674 17.0008 0.690674ZM17.0008 31.5032C9.03203 31.5032 2.55078 24.9688 2.55078 17C2.55078 9.0313 9.03203 2.55005 17.0008 2.55005C24.9695 2.55005 31.5039 9.0313 31.5039 17C31.5039 24.9688 24.9695 31.5032 17.0008 31.5032Z" />
              <path d="M17.9039 6.32194C16.3633 6.05631 14.8227 6.48131 13.707 7.43756C12.5383 8.39381 11.8477 9.82819 11.8477 11.3688C11.8477 11.9532 11.9539 12.5376 12.1664 13.0688C12.3258 13.5469 12.857 13.8126 13.3352 13.6532C13.8133 13.4938 14.0789 12.9626 13.9195 12.4844C13.8133 12.1126 13.707 11.7938 13.707 11.3688C13.707 10.4126 14.132 9.50944 14.8758 8.87194C15.6195 8.23444 16.5758 7.96881 17.5852 8.18131C18.9133 8.39381 19.9758 9.50944 20.1883 10.7844C20.4539 12.3251 19.657 13.8126 18.2227 14.3969C16.8945 14.9282 16.0445 16.2563 16.0445 17.7969V21.1969C16.0445 21.7282 16.4695 22.1532 17.0008 22.1532C17.532 22.1532 17.957 21.7282 17.957 21.1969V17.7969C17.957 17.0532 18.382 16.3626 18.9664 16.1501C21.1977 15.2469 22.4727 12.9094 22.0477 10.4657C21.6758 8.39381 19.9758 6.69381 17.9039 6.32194Z" />
              <path d="M17.0531 24.8625H16.8937C16.3625 24.8625 15.9375 25.2875 15.9375 25.8188C15.9375 26.35 16.3625 26.7751 16.8937 26.7751H17.0531C17.5844 26.7751 18.0094 26.35 18.0094 25.8188C18.0094 25.2875 17.5844 24.8625 17.0531 24.8625Z" />
            </svg>
          </div>
          <div className="w-full">
            <h3 className="mb-6 text-xl font-semibold sm:text-2xl lg:text-xl xl:text-2xl">
              {question}
            </h3>
            <p className="text-base">{answer}</p>
          </div>
        </div>
      </>
    );
  };
  